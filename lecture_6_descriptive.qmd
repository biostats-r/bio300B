...
p_dist + 
  geom_segment(aes(x = value, xend = value, y = 5, yend = 0, colour = name), data = mid, arrow = arrow(type = "closed")) +
...
animate(width = 500, height = 300, median_anim)
```
...
max_min <- penguins |> 
  filter(species == "Gentoo") |> 
  drop_na() |> 
  summarise(minimum  = min(bill_length_mm),
...
dat2 <- bind_rows(
  small = rerun(1000, slice_sample(dat, n = 20)) |> bind_rows(.id = "run"),
  large = rerun(1000, slice_sample(dat, n = 100)) |> bind_rows(.id = "run"),
.id = "size") |> 
...
correct <-"#a0ffa0"
error <- "orange"
tibble(`Null hypothesis is` = c("Rejected", "Not rejected"), 
...
server <- function(input, output, session) {
  output$distPlot <- renderPlot({
    ...
    par(oma=c(2,2,0,0),mar=c(1.2,2,2,1),mfrow=c(2,1), cex = 1.5, tcl = -0.1, mgp = c(3, .2, 0))
