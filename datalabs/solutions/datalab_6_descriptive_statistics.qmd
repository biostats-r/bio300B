---
title: "Datalab 6: descriptive statistics - solution"
date: today
format: html
---
...
#| label: setup
#| message: false
#activate packages - best done at top of script
library(tidyverse)
...
```{r}
#| label: fig-adelie-plot

adelie |> 
  ggplot(aes(x = sex, 
             y = bill_length_mm, 
             colour = sex, 
             fill = after_scale(colorspace::lighten(colour, 0.8)))) + # automatically make fill the same colour as the colour, but slightly lighter
...
# Colour & fill not necessary for a ms, but nice in a presentation.  
```
...
Male Adelie (n = `r adelie_summary |> filter(sex == "male") |> pull(n)`) have longer (mean = `r mod$estimate["mean in group male"]`, sd = `r adelie_summary |> filter(sex == "male") |> pull(sd)`) bills, than female Adelie (n = `r adelie_summary |> filter(sex == "female") |> pull(n)`, mean = `r mod$estimate["mean in group female"]`, sd = `r adelie_summary |> filter(sex == "female") |> pull(sd)`).
This difference is significant t(`r mod$parameter`) = `r mod$statistic`, p = `r format.pval(mod$p.value, eps = 0.001)`.
