...
```{r diff-models-reg, fig.height=8, echo = FALSE, fig.width  = 10}
adelie <- penguins |> 
...
preds <- bind_rows(
  ...
  "y ~ x + z", "Continuous and categorical predictors",
  "y ~ x * z", "Interaction")) |> 
  mutate(model = factor(model, levels = c("y ~ 1", "y ~ x", "y ~ z", "y ~ x + z", "y ~ x * z")),
         type = factor(type, levels = c("Intercept only", "Continuous predictor", "Categorical predictor", "Continuous and categorical predictors", "Interaction")))
...
```{r diff-models-anova, fig.height=8, echo = FALSE, fig.width  = 10}
library(ggbeeswarm)
...
p1 <- base_plot +
  geom_pointrange(
    ...
    colour = "red",
    size = 1.1) +
  ggtitle("y ~ 1", subtitle = "Intercept only")
...
p3 <- base_plot +
  ...
  geom_pointrange(
    ...
    size = 1.1) +
  ggtitle("y ~ x + z", subtitle = "Two-way anova")
...
p4 <- base_plot +
  ...
  geom_pointrange(
    ...
    position = position_dodge(width = 1),
    size = 1.1) +
  ggtitle("y ~ x * z", subtitle = "Interaction")
...
```{r interaction, fig.width = 8, fig.height = 6, echo = FALSE}
...
```{r cont-int, echo = FALSE, fig.width = 10, fig.height= 6}
...
```{r cont-cont, echo = FALSE, fig.width = 8, fig.height = 6}
